...
type wallet struct {
	...
	deriveChain    timenewbank.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived Mit account
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain timenewbank.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
