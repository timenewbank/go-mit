...
#
# The cluster contains a bootnode, a mit node and multiple swarm nodes, with
...
# start_bootnode starts a bootnode which is used to bootstrap the mit and
# swarm nodes
start_bootnode() {
...
# start_geth_node starts a mit node with --datadir pointing at <base-dir>/geth
...
start_geth_node() {
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get mit account address"
  fi
...
create_account() {
  ...
  mit --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
